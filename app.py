...
class DOIProcessor:
    ...
    def _find_bibliographic_doi(self, reference: str) -> Optional[str]:
        ...
        Использует обязательные 4 параметра: журнал, год, том, номер_статьи/страницы.
        Проверяет первого автора для валидации.
        """
        try:
            # 1. Парсинг библиографической ссылки
            ...
            if not all([journal, year, volume]):
                logger.warning(f"Недостаточно обязательных данных: {parsed_data}")
                return None
            
            if not (article_num or first_page):
                logger.warning(f"Нет номера статьи или страницы: {parsed_data}")
                return None
            
            # 3. Формирование точного запроса
            query_conditions = []
            
            # Обязательные условия с логикой И
            query_conditions.append(f'container-title:"{journal}"')
            
            # Для года используем диапазон
            query_conditions.append(f'from-pub-date:{year}-01-01')
            query_conditions.append(f'until-pub-date:{year}-12-31')
            
            # Том обязателен
            query_conditions.append(f'volume:{volume}')
            
            # ИЛИ для номера статьи и страницы
            number_conditions = []
            if article_num:
                number_conditions.append(f'article-number:{article_num}')
            if first_page:
                number_conditions.append(f'page:{first_page}')
            
            if number_conditions:
                query_conditions.append(f"({' OR '.join(number_conditions)})")
            
            # Полный запрос
            query_string = ",".join(query_conditions)
            logger.info(f"Поисковый запрос: {query_string}")
            ...
            # 5. Анализ результатов с системой баллов
            for result in results:
                if 'DOI' not in result:
                    continue
                
                score = 0
                doi_candidate = result['DOI']
                
                # КРИТИЧЕСКИЕ ПРОВЕРКИ (максимальные баллы)
                
                # A) Проверка журнала (40 баллов)
                journal_match = False
                # Полное название
                container_titles = result.get('container-title', [])
                for title in container_titles:
                    if title and journal.lower() in title.lower():
                        score += 30
                        journal_match = True
                        break
                ...
                if not journal_match:
                    short_titles = result.get('short-container-title', [])
                ...
                if not journal_match:
                    continue  # Пропускаем, если журнал не совпадает
                
                # B) Проверка года (30 баллов) - ОБЯЗАТЕЛЬНО
                result_year = self._extract_year_from_result(result)
                if result_year == year:
                    score += 30
                else:
                    continue  # Год должен совпадать точно
                
                # C) Проверка тома (20 баллов) - ОБЯЗАТЕЛЬНО
                result_volume = result.get('volume', '')
                if str(result_volume) == str(volume):
                    score += 20
                else:
                    continue  # Том должен совпадать
                
                # D) Проверка номера статьи или страницы (10 баллов) - ОБЯЗАТЕЛЬНО
                number_match = False
                
                # Проверяем номер статьи
                result_article_num = result.get('article-number', '')
                if article_num and str(article_num) == str(result_article_num):
                    score += 10
                    number_match = True
                
                # Проверяем первую страницу
                if not number_match and first_page:
                    result_pages = result.get('page', '')
                    if result_pages:
                        # Обработка разных форматов: "157", "157-165", "157–165"
                        if '-' in result_pages:
                            result_first_page = result_pages.split('-')[0].strip()
                        elif '–' in result_pages:
                            result_first_page = result_pages.split('–')[0].strip()
                        else:
                            result_first_page = result_pages.strip()
                        
                        if str(first_page) == str(result_first_page):
                            score += 10
                            number_match = True
                
                if not number_match:
                    continue  # Номер/страница должны совпадать
                
                # E) ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: первый автор (бонусные баллы)
                if first_author and result.get('author'):
                    result_authors = result['author']
                    if result_authors:
                        result_first_author = result_authors[0].get('family', '').lower()
                        input_first_author = first_author.lower()
                        
                        # Проверяем частичное совпадение (для обработки Sadykova/Sadykov)
                        if (input_first_author == result_first_author or 
                            input_first_author[:-1] == result_first_author or  # Sadykova -> Sadykov
                            result_first_author[:-1] == input_first_author):   # Sadykov -> Sadykova
                            score += 50  # Максимальный бонус за автора
                            logger.info(f"✓ Совпадение автора: {first_author} == {result_first_author}")
                
                # ИТОГ: минимальный порог - 100 баллов (все обязательные поля + журнал)
                if score >= 100 and score > best_score:
                    best_score = score
                    best_match = doi_candidate
                    logger.debug(f"Кандидат: {doi_candidate}, баллы: {score}")
            
            # 6. Возврат результата
            if best_match and best_score >= 100:
                logger.info(f"✅ Найден DOI {best_match} с {best_score} баллами")
                return best_match
            else:
                logger.warning(f"❌ Не найдено точного совпадения. Лучший результат: {best_score if best_match else 0} баллов")
                return None
    ...
    def _parse_bibliographic_reference(self, reference: str) -> Optional[Dict]:
        """
        Улучшенный парсинг библиографических ссылок.
        Обрабатывает форматы:
        - "E. Ruiz-Trejo and J. T. S. Irvine, Solid State Ionics, 2013, 252, 157"
        - "V. Sadykova et al., Chim. Tech. Acta, 2025, 12, 12304"
        - "Solid State Ionics, 2013, 252, 157"
        """
        try:
            ...
            # ШАГ 1: Извлечение первого автора (если есть)
            # Форматы: "E. Ruiz-Trejo and J. T. S. Irvine, " или "V. Sadykova et al., "
            author_patterns = [
                r'^([A-Z]\.\s+[A-Z][a-z]+(?:\s+and\s+[A-Z].*?)?)(?:,| et al\.)',
                r'^([A-Z][a-z]+,\s+[A-Z]\.(?:\s+[A-Z]\.)?)(?:,| et al\.)',
                r'^([A-Z][a-z]+\s+[A-Z]\.)'
            ]
            
            for pattern in author_patterns:
                match = re.match(pattern, clean_ref)
                if match:
                    author_text = match.group(1).strip()
                    # Извлекаем фамилию первого автора
                    parts = re.split(r'[,\s]+', author_text)
                    for part in parts:
                        if (part and len(part) > 1 and 
                            '.' not in part and 
                            part.lower() not in ['and', '&']):
                            result['first_author'] = part
                            break
                    clean_ref = clean_ref[match.end():].strip()
                    break
            
            # ШАГ 2: Поиск года (4 цифры)
            year_match = re.search(r'\b(19|20)\d{2}\b', clean_ref)
            if year_match:
                result['year'] = int(year_match.group())
                # Удаляем год из строки для дальнейшего парсинга
                clean_ref = clean_ref[:year_match.start()] + clean_ref[year_match.end():]
            
            # ШАГ 3: Поиск журнала (текст до года или до чисел)
            # Ищем текст до первой цифры или до запятой перед годом
            journal_patterns = [
                r'^(.*?)(?=,\s*\d|$)',
                r'^(.*?)(?=\s*\d|$)'
            ]
            ...
            for pattern in journal_patterns:
                ...
                if match:
                    ...
                    if journal_text and len(journal_text) > 3:
                        result['journal'] = journal_text
                        clean_ref = clean_ref[match.end():].strip()
                        break
            
            # ШАГ 4: Поиск тома и номера/страницы
            # Извлекаем все числа из оставшейся строки
            numbers = re.findall(r'\b(\d{1,6})\b', clean_ref)
            
            if numbers:
                # Первое число после журнала - том
                result['volume'] = numbers[0]
                
                # Второе число - номер статьи или страница
                if len(numbers) >= 2:
                    number = numbers[1]
                    
                    # Эвристика для определения типа числа:
                    # 1) Если есть явное указание "Art." или "Article"
                    art_match = re.search(r'(?:Art\.|Article|art\.)\s*(\d+)', reference, re.IGNORECASE)
                    if art_match:
                        result['article_number'] = art_match.group(1)
                    # 2) Если число больше 1000, скорее всего номер статьи
                    elif int(number) > 1000:
                        result['article_number'] = number
                    # 3) Иначе считаем это страницей
                    else:
                        result['first_page'] = number
            
            # Дополнительные проверки для специальных форматов
            # Формат "V. 12. Art. 12304"
            if not result['article_number']:
                art_match = re.search(r'[Vv]\.\s*\d+[^.]*Art\.\s*(\d+)', reference, re.IGNORECASE)
                if art_match:
                    result['article_number'] = art_match.group(1)
            
            # Формат "Vol. 252, pp. 157-165"
            if not result['first_page']:
                pp_match = re.search(r'[Pp]p?\.?\s*(\d+)(?:[-\–]\d+)?', reference)
                if pp_match:
                    result['first_page'] = pp_match.group(1)
            
            logger.debug(f"Парсинг: '{reference[:80]}...' -> {result}")
            return result if any(result.values()) else None
            
        except Exception as e:
            logger.error(f"Ошибка парсинга ссылки '{reference[:100]}...': {e}")
            return None
    
    def _extract_year_from_result(self, result: Dict) -> Optional[int]:
        """
        Извлечение года публикации с приоритетом для printed версии.
        """
        # Порядок приоритета полей для года
        year_sources = [
        ...
        for field, subfield in year_sources:
